...
namespace GSAMSistemaVentas.UI
{
    public partial class ProveedorDetalleUI : Form
    {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                ProveedorID = lblID.Text.Length == 0 ? 0 : int.Parse(lblID.Text),
                RazonSocial = txtRazonSocial.Text.Trim().ToUpper(),
                RUC = txtRuc.Text.Trim().ToUpper(),
                Direccion = txtDireccion.Text.Trim().ToUpper(),
                Telf = txtTelf.Text.Trim().ToUpper(),
                Mail = txtCorreo.Text.Trim().ToUpper(),
            ...
            {
                rpta = new ProveedorLogic().Insert(obj);
            }

            if (Operacion == MisConstantes.CRUD.Modificacion)
            {
                rpta = new ProveedorLogic().Update(obj);
            }
